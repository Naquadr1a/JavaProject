...
public class Box <T extends Fruit> {
    ...
    void addFruit(T fruit, int count) {
        fruitList.add(fruit);
            totalWeight = count * fruit.weight;
    }
    ...
    //Метод для сравнивания веса коробок
    ...
    // Метод для перекладывания фруктов из коробки в коробку
    void sprinkle (Box box) {
        for (int i = 0; i < fruitList.size(); i++) {
        box.fruitList.add(this.fruitList.get(i));
